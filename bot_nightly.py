...
async def on_ready():
    ...
    for guild in range(len(client.guilds)):
        if client.guilds[guild].name == None:
            ...
            try:
                with open(rf"../pengaelicbotnightly.data/configs/{client.guilds[guild].id}.json", "r") as optionsfile:
            ...
            # if something goes wrong...
                ...
                # ...make a blank file...
                try:
                    open(rf"../pengaelicbotnightly.data/configs/{client.guilds[guild].id}.json", "x").close()
                except FileExistsError:
                    pass
                # ...and try restoring from the main build
                try:
                    with open(rf"../pengaelicbot.data/configs/{client.guilds[guild].id}.json", "r") as originalfile:
                        allOptions = load(originalfile)
                    with open(rf"../pengaelicbotnightly.data/configs/{client.guilds[guild].id}.json", "w") as optionsfile:
                        dump(allOptions, optionsfile, sort_keys=True, indent=4)
                    print("Options file restored for " + str(client.guilds[guild].name))
                # if something STILL goes wrong...
                except:
                    # ...restore the default options file
...
async def on_guild_join(guild, ctx=None):
    ...
    welcomeEmbed = discord.Embed(title="Howdy fellas! I'm (the dev version of) the Pengaelic Bot!", description="Type `pn!help` for a list of commands.", color=32639)
    ...
    try:
        open(rf"../pengaelicbotnightly.data/configs/{guild.id}.json", "x").close()
        with open(r"default_options.json", "r") as defaultsfile:
            allOptions = load(defaultsfile)
        with open(rf"../pengaelicbotnightly.data/configs/{guild.id}.json", "w") as optionsfile:
            dump(allOptions, optionsfile, sort_keys=True, indent=4)
...
async def on_command_error(ctx, error):
    ...
    with open(rf"../pengaelicbotnightly.data/configs/{ctx.guild.id}.json", "r") as optionsfile:
        allOptions = load(optionsfile)
...
async def help(ctx, category=None, subcategory=None):
    cyan = 32639
    helpMenu = discord.Embed(title=client.description, description="Type `pn!help <category name>` for more info on each category.", color=cyan)
    cogfiles = [cog[:-3] for cog in listdir("./cogs.nightly") if cog.endswith(".py")]
