...
class Games(commands.Cog):
	...
	async def roll_dice(self, ctx, *, ds: str = "1d6"):
		try:
			...
			if	dice == 0:	response = "You didn't roll any dice."
				...
				if dice > 1:
					if len(str(roll_results[:-1])[1:-1]) < 2000:	response = f"You rolled {list2str(roll_results, 0, True)}, totalling {total}"
					else:	response = f"You rolled a total of {total}"

				else:	response = "You rolled " + str(total)

		except ValueError:	response = "Invalid dice syntax. Please use `[count]d[sides]`, like D&D (e.g. `2d6`, `1d20`)."
	...
	async def hangman(self, ctx, pengaelic=None):
		words	= hangman_words
		...
		while chances != 0 and flag == False:	# flag is updated when word is correctly guessed
			...
			if user:	guess = regional_indicators[reaction.emoji]
			# validation of guess
			...
			for char in word:
				...
				else:	word_to_print += "_"
			await the_word.edit(content=" ".join(list(word_to_print)).replace("_", "\_"))

		# if player has used all of their chances
		if chances <= 0 and (Counter(letter_guessed) != Counter(word)):	await chance_counter.edit(content=f'You lost!\nThe word was "{word}".')
	...
	async def error(self, ctx, error):
		...
		elif error == "HTTPException: 400 Bad Request (error code: 50006): Cannot send an empty message":	"for some reason, hangman throws this error when nothing is even really supposed to happen in the first place, this string is here to merely disregard it"
